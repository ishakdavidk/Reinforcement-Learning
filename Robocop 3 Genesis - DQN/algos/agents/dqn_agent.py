...
class DQNAgent():
    def __init__(self, input_shape, action_size, seed, device, buffer_size, batch_size, gamma, lr, tau, update_every, replay_after, model, TRAIN, model_epoch):
        ...
        if not TRAIN:
            policy_net_file = Path('model/model_dqn/' + model_epoch + '_policy.pt')
            target_net_file = Path('model/model_dqn/' + model_epoch + '_target.pt')

            if not policy_net_file.is_file() or not target_net_file.is_file():
                sys.exit('\nModel not found, please check the model name by adding \'--epoch_model\' or train the model first by adding \'--train\' argument.')
            self.policy_net.load_state_dict(torch.load(policy_net_file))
            self.target_net.load_state_dict(torch.load(target_net_file))
